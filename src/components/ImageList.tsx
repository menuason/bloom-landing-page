...
interface ImageListProps {
  images: Image[];
  className?: string;
  isForInlineSlider?: boolean;
}

export const ImageList: FC<ImageListProps> = ({ images, isForInlineSlider, className }) => {
  ...
  const handleNext = () => {
    if (selectedImage && selectedImage.sliderImages) {
      ...
      setSelectedImage({
        ...selectedImage,
        sliderImg:
          selectedImage.sliderImages.images[
            currentIndex === selectedImage.sliderImages.images.length - 1
              ? 0
              : currentIndex + 1
            ],
      });
